"""13- Faça um algoritmo que calcule a média ponderada das notas de 3 provas.
A primeira e segunda prova tem peso 1 e a terceira tem peso 2. Ao final,
mostrar a média do aluno e indicar se o aluno foi aprovado ou reprovado. A
nota para aprovação deve ser igual ou superior a 60 pontos."""

nota1 = float(input("Nota da primeira prova (de 0 a 100): "))
nota2 = float(input("Nota da segunda prova (de 0 a 100): "))
nota3 = float(input("Nota da terceira prova (de 0 a 100): "))
media = (nota1 + nota2 + (2 * nota3)) / 4

if media >= 60:
    print(f"Aluno aprovado com média {media:.2f}")
else:
    print(f"Aluno reprovado, média {media:.2f}")
